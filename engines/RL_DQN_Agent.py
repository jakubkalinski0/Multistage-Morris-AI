...
class RLDQNAgent:
    DEFAULT_INPUT_SIZE_3MM = 9 * 3 + 1 + 2 + 2 + 1 # Reference

    def __init__(self, board: Board, model_path: Optional[str] = None,
        ...
        # If model_path is None, create a default path based on board_size
        self.model_path_from_init = model_path if model_path is not None else os.path.join("models", f"{self.board_size}mm_rl_dqn_model.pth")
        ...
        print(f"RLDQNAgent for {self.board_size}MM using device: {self.device}")
        ...
        # Use the potentially board-specific model_path
        current_model_path_to_load = self.model_path_from_init

        if current_model_path_to_load: # It will always be a string now
            model_dir = os.path.dirname(current_model_path_to_load)
            if model_dir and not os.path.exists(model_dir):
                try: os.makedirs(model_dir, exist_ok=True)
                except OSError as e: print(f"Could not create model dir: {e}")
            if os.path.exists(current_model_path_to_load):
                try:
                    self.policy_net.load_state_dict(torch.load(current_model_path_to_load, map_location=self.device))
                    self.model_loaded = True; print(f"Loaded RL DQN model: {current_model_path_to_load}")
                except Exception as e: print(f"Error loading RL DQN model {current_model_path_to_load}: {e}. Fresh weights.")
            else: print(f"RL DQN model not found: {current_model_path_to_load}. Fresh weights.")
        # No 'else' needed as current_model_path_to_load is always defined
    ...
    # ... (Reszta metod: _get_max_possible_actions_for_board, _initialize_action_space_map, ...)
    # ... (_get_action_index, _get_move_from_index, _state_to_tensor, ...)
    # ... (select_action_epsilon_greedy, get_best_move, optimize_model, ...)
    # ... (update_target_network_if_needed, save_model) ...
    # --- Te metody pozostają takie same jak w OSTATNIEJ PEŁNEJ WERSJI RLDQNAgent ---
    ...
    def _initialize_action_space_map(self):
        ...
        for r in range(self.board_size): # Move/Fly
            for c in range(self.board_size):
                ...
                self.action_to_move_map[idx] = move; self.move_to_action_map[move] = idx; idx += 1
        if idx != self.num_actions: print(f"CRITICAL: Action space init mismatch. Expected {self.num_actions}, generated {idx}.")
    ...
    def select_action_epsilon_greedy(self, state: BoardState) -> Tuple[Optional[Move], Optional[int]]:
        ...
        if random.random() < eps_threshold or not self.model_loaded:
            ...
            if action_idx is None:
                # print(f"Warning: Random legal move {chosen_move} not in action map during exploration.")
                for m_idx_loop, m_obj_loop in self.action_to_move_map.items():
                    if m_obj_loop in legal_moves: chosen_move = m_obj_loop; action_idx = m_idx_loop; break
    ...
    def get_best_move(self, state: BoardState) -> Optional[Move]:
        ...
        with torch.no_grad():
            ...
            for move in legal_moves:
                ...
                if action_idx is not None and 0 <= action_idx < self.num_actions: # Corrected variable from idx to action_idx
    ...
    def save_model(self, path: Optional[str] = None):
        ...
        # Default save_path if self.model_path_from_init was None (should be set now)
        if not save_path: save_path = os.path.join("models", f"rl_dqn_{self.board_size}mm_ep{self.training_episodes_count}.pth")
