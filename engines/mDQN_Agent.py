...
class BaseSubAgent:
    ...
    def __init__(self, board_size: int, num_specific_actions: int, model_filename_suffix: str,
        ...
        self.model_path_from_init = os.path.join("../models",
                                                 f"3mm_mdqn_{model_filename_suffix}.pth")  # Specific to 3MM for now
        if board_size != 9:  # Adjust path if not 3MM
            self.model_path_from_init = os.path.join("../models", f"{board_size}mm_mdqn_{model_filename_suffix}.pth")
        ...
        if model_dir and not os.path.exists(model_dir):
            try:
                ...
                print(f"Could not create model dir: {e}")
        ...
        if os.path.exists(self.model_path_from_init):
            try:
                ...
                print(f"Loaded model for {model_filename_suffix} from {self.model_path_from_init}")
            except Exception as e:
                print(f"Error loading {model_filename_suffix} model: {e}. Fresh weights.")
        else:
            print(f"{model_filename_suffix} model not found at {self.model_path_from_init}. Fresh weights.")
        ...
        self.steps_done = 0  # Will be set by main MDQNAgent
        self.sub_agent_training_episodes_count = 0  # For its own target updates
    ...
    def _state_to_tensor(self, state: BoardState) -> torch.Tensor:
        ...
        pl_w = float(state.pieces_left_to_place_by_player[Player.WHITE]);
        pl_b = float(state.pieces_left_to_place_by_player[Player.BLACK])
        po_w = float(state.pieces_from_player_currently_on_board[Player.WHITE]);
        po_b = float(state.pieces_from_player_currently_on_board[Player.BLACK])
    ...
    def optimize_model_step(self):
        ...
        non_final_mask = torch.tensor(tuple(s is not None for s in next_state_batch_list), dtype=torch.bool).to(
            self.device)
        ...
        if len(non_final_next_states_list) > 0:
            ...
            with torch.no_grad():
                ...
                self.target_net(non_final_next_states_tensor).max(1, keepdim=True)[0]
    ...
    def update_target_network_if_needed(self, global_episode_count):
        if global_episode_count % self.target_update_freq == 0:
            self.target_net.load_state_dict(self.policy_net.state_dict())
            # print(f"Target net for {self.model_filename_suffix} updated at ep {global_episode_count}")
    ...
    def save_model_sub(self):
        ...
        print(f"{self.model_filename_suffix} model saved to {self.model_path_from_init}")


class PlacementAgentDQN(BaseSubAgent):
    def __init__(self, board_size, device, **kwargs):
        super().__init__(board_size, board_size, "placement", device, **kwargs)
...
class MovementAgentDQN(BaseSubAgent):
    def __init__(self, board_size, device, **kwargs):
        num_move_actions = board_size * (board_size - 1)
        super().__init__(board_size, num_move_actions, "movement", device, **kwargs)

    def get_action_index(self, move: Move) -> Optional[int]:
        if move.move_type == MoveType.MOVE:
            from_id = move.from_position.id;
            to_id = move.to_position.id
            to_idx_adjusted = to_id if to_id < from_id else to_id - 1
            return from_id * (self.board_size - 1) + to_idx_adjusted
        return None

    def get_move_from_index(self, action_index: int) -> Optional[Move]:
        if 0 <= action_index < self.num_actions:
            from_id = action_index // (self.board_size - 1)
            to_idx_adjusted = action_index % (self.board_size - 1)
            to_id = to_idx_adjusted if to_idx_adjusted < from_id else to_idx_adjusted + 1
...
class RemovalAgentDQN(BaseSubAgent):
    ...
    def get_move_from_index(self, action_index: int) -> Optional[Move]:
        if 0 <= action_index < self.board_size: return Move(MoveType.REMOVE,
                                                            remove_checker_from_position=Position(action_index))
        return None
...
class MDQNAgent:
    def __init__(self, board: Board, device: Optional[str] = None, **hyperparams):
        ...
        print(f"MDQNAgent using device: {self.device}")
        default_hyperparams = {
            'learning_rate': 0.00025, 'gamma': 0.99, 'epsilon_start': 1.0,
            'epsilon_end': 0.05, 'epsilon_decay': 30000, 'target_update_freq': 20,
            'replay_memory_capacity': 30000, 'batch_size': 128
        }
        ...
        self.steps_done = 0  # Shared for epsilon decay calculation across sub-agents
        self.training_episodes_count = 0  # For periodic target updates
    ...
    def _get_current_sub_agent_key_and_instance(self, state: BoardState) -> Tuple[str, BaseSubAgent]:
        ...
        if current_phase == GamePhase.PLACEMENT:
            return "placement", self.placement_agent
        elif current_phase in [GamePhase.MOVEMENT, GamePhase.FLYING]:
            return "movement", self.movement_agent
    ...
    def select_action_epsilon_greedy(self, state: BoardState) -> Tuple[Optional[Move], Optional[int]]:
        ...
        sub_agent.steps_done = self.steps_done  # Use main agent's step counter
        ...
        # Note: self.steps_done is incremented in the main training loop after calling this

        if random.random() < eps_threshold or not sub_agent.model_loaded:
            chosen_move = random.choice(legal_moves)
            action_idx = sub_agent.get_action_index(chosen_move)
            if action_idx is None:  # Fallback if chosen legal move can't be mapped by this sub-agent
                for m in legal_moves:  # Try to find any mappable legal move
                    idx_try = sub_agent.get_action_index(m)
                    if idx_try is not None: chosen_move = m; action_idx = idx_try; break
                if action_idx is None: return None, None  # Should not happen if legal moves exist for phase
                ...
                for move in legal_moves:
                    ...
                    if idx is not None and 0 <= idx < sub_agent.num_actions:
                        if q_values[idx].item() > best_q_val:
                            best_q_val = q_values[idx].item();
                            best_move = move;
                            best_idx = idx
                if best_move:
                    return best_move, best_idx
                else:
                    chosen_move = random.choice(legal_moves); return chosen_move, sub_agent.get_action_index(
                        chosen_move)
    ...
    def optimize_sub_agent(self, sub_agent_key: str):
        self.sub_agents[sub_agent_key].optimize_model_step()

    def update_target_networks_globally(self):  # Call this once per episode from training loop
    ...
    def save_models(self):
        for _, agent_instance in self.sub_agents.items():
            agent_instance.save_model_sub()
        print("All mDQN sub-agent models saved.")

    def push_experience(self, state_tensor, action_idx, reward, next_state_tensor, done,
                        state_for_sub_agent_check: BoardState):
